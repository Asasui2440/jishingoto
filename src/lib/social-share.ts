...
export const APP_SHARE_URL = "https://jishingoto-rouge.vercel.app";
...
export function quizShareText(scores: Record<Axis, number | null>, mock = false) {
  ...
  return ["部屋の安全をチェックしました！", "行動クイズの結果", ...rows, ...(rows.length ? [] : ["行動クイズ：未回答"]), ...(mock ? ["添付の予想図はサンプルです。自分の部屋の再現ではありません。"] : []), "#ジシンゴト"].join("\n");
}
