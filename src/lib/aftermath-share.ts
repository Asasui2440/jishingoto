...
export async function createAftermathShareFile(url: string, mock = false, comparisonUnavailable = false): Promise<File> {
  ...
  context.fillText(mock ? "サンプルの予想図 · あなたの部屋の再現ではありません" : comparisonUnavailable ? "AIによる予想図 · 元写真との自動比較は未完了" : "AIによる予想図 · 実際の被害写真ではありません", width / 2, height + footer * 0.44);
  ...
  return new File([blob], mock ? "jishingoto-sample-room.png" : "jishingoto-ai-room.png", { type: "image/png" });
}
