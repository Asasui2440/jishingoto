...
function SetupMap({ mode, initialRoutes, settingsOpen, setSettingsOpen, selectMode }: {
  ...
  useEffect(() => {
    ...
    const lookup = async () => {
      ...
      return groupSchoolShelters(lists.flat(), spot).map(s => ({ ...s, kind: "指定緊急避難場所" }))
  ...
  return <>
    ...
    <main className="flex min-h-0 flex-1 flex-col">
      {stage === "routes" ? <div className="mx-4 mb-2 shrink-0"><label className="flex shrink-0 items-center gap-3 rounded-xl bg-primary px-2 py-1 text-[10px] font-bold">災害ケース<select aria-label="災害ケース" value={scenario} onChange={e => { if (isEvacScenario(e.target.value)) changeScenario(e.target.value); }} className="min-h-11 w-24 bg-transparent text-13">{Object.entries(EVAC_SCENARIOS).map(([value, item]) => <option key={value} value={value}>{item.label}</option>)}</select></label></div> : null}
    ...
    <BottomSheet open={sheet === "route"} title="経路の詳細" onClose={() => setSheet(null)}>
      {active ? <div className="space-y-3"><p className="text-base font-bold"><Furigana text={active.label} /></p><p>{scenarioInfo.label}の避難先への徒歩経路 · {formatDistance(active.distanceM)} · {formatDuration(active.durationS)}</p>{scenario === "flood" ? <p className="rounded-xl bg-blue-50 p-3 text-13 text-blue-900">{active.assessment?.flood?.status === "available" ? "洪水の浸水想定を参照して比較しています。表示は想定最大規模で、現在の冠水・通行状況ではありません。" : "洪水の浸水想定を確認できない区間があります。詳細の取得状況を確認してください。"}<a href={FLOOD_SOURCE} target="_blank" rel="noreferrer" className="block underline">出典：重ねるハザードマップ（国土交通省等）</a></p> : null}<div className="space-y-2">{active.notes.map(n => <p key={n} className="text-13 text-ink-muted"><Furigana text={n} /></p>)}</div><p className="rounded-2xl bg-primary-soft p-3 text-11">{active.demo ? "練習用の経路です。実際の道路とは異なります。" : "距離や地形の比較は学習用です。安全性や現在の通行状況を保証するものではありません。"}</p>{mode === "api" ? <p className="text-13 text-ink-muted">{analysisMode === "sample" ? "実際の道で固定の練習問題を体験します。" : "災害ケースと周辺の土地利用に合う想定問題を出題します。不足分は共通問題で補います。"}</p> : null}</div> : null}
    </BottomSheet>
  </>;
}
