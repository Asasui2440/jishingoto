...
function SetupMap({ mode, initialRoutes, settingsOpen, setSettingsOpen, selectMode }: {
  ...
  const loadRoutes = useCallback(async () => {
    const state = getEvac();
    if (!state.home || !state.shelter) { if (initialRoutes) router.replace("/evac"); return; }
  ...
  return <>
    ...
    <main className="flex min-h-0 flex-1 flex-col">
      <div role="group" aria-label="体験モード切り替え" className="mx-4 mb-2 flex shrink-0 gap-1 rounded-xl border border-border bg-white p-1">
        {(["mock", "api"] as const).map(value => <button key={value} type="button" aria-pressed={mode === value} onClick={() => selectMode(value)} className={`min-h-9 flex-1 rounded-lg text-11 font-bold ${mode === value ? "bg-primary text-ink" : "bg-white text-ink-muted"}`}>{value === "api" ? "API版・実際の街" : "サンプル版"}</button>)}
      </div>
