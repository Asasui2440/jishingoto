...
export async function POST(request: Request) {
  ...
  form.set("prompt", "入力写真と同じ部屋・同じ視点・家具の元の位置関係を保ち、強い地震の直後を想像した教育用のアニメ背景画にしてください。写真に実際に見える家具や物だけを手がかりに、転倒・落下、割れものの破損、物による通路のふさがりが起こりうる様子を、怖すぎない全年齢向けの日本アニメ背景美術として描いてください。写っていない家具や窓、危険物を追加しないでください。マスクで隠された領域は無地のまま保ち、内容を推測・復元しないでください。人物、文字、ロゴ、炎、流血、けが人は描かないでください。固定状態や実際の被害は写真では断定できません。これは被害の予測ではなく、備えを考えるための一例です。");
