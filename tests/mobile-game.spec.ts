...
test("API-free sample follows privacy, room quiz, sharing and evacuation to the final report", async ({ page }, testInfo) => {
  const apiCalls: string[] = [];
  await page.route("**/api/**", (route) => { apiCalls.push(route.request().url()); return route.abort(); });
  await page.route(/^https:\/\//, (route) => route.abort());
  await page.goto("/");
  await page.getByRole("button", { name: "サンプルで最後まで体験する（API不要）", exact: true }).click();
  await expect(page.getByRole("link", { name: "サンプルデータを保存" })).toHaveAttribute("href", "/demo/room.json");
  await page.getByRole("button", { name: "サンプル体験を始める", exact: true }).click();
  await expect(page).toHaveURL(/\/privacy$/);
  await page.getByRole("button", { name: "OK、このまますすむ" }).click();
  await expect(page).toHaveURL(/\/risks$/);
  await page.getByRole("button", { name: /このまま.*体験.*始/ }).click();
  await expect(page).toHaveURL(/\/quiz$/);
  while (new URL(page.url()).pathname === "/quiz") {
    const choices = page.getByRole("list").getByRole("button");
    await choices.first().click();
    await page.waitForTimeout(400);
  }
  await expect(page).toHaveURL(/\/result$/);
  await expect(page.getByText("APIなしのテスト用画像です。生成結果の代わりに固定の元写真を表示しています。")).toBeVisible();
  await captureStage(page, testInfo, "sample-room-result");
  while (await page.getByRole("button", { name: "次へ", exact: true }).count()) {
    await page.getByRole("button", { name: "次へ", exact: true }).click();
  }
  await page.getByRole("button", { name: /部屋.*結果.*シェア/ }).click();
  await expect(page).toHaveURL(/\/share$/);
  await expect(page.locator("canvas")).toBeAttached();
  await page.getByRole("button", { name: /避難.*ルートへ.*進/ }).click();
  await expect(page).toHaveURL(/mode=mock/);
  const places = page.getByRole("region", { name: "避難先を選ぶ", exact: true });
  await places.getByRole("button", { name: /教育.*森公園/ }).click();
  await page.getByRole("button", { name: "ルートを比べる", exact: true }).click();
  await page.getByRole("button", { name: "この道でスタート", exact: true }).click();
  ...
  await page.getByRole("button", { name: "次の判断ポイントへ進む", exact: true }).click();
  await page.getByRole("button", { name: "ふりかえる", exact: true }).click();
  await expect(page).toHaveURL(/\/evac\/report$/);
  await expect(page.getByRole("heading", { name: "ふりかえり", exact: true })).toBeVisible();
  await captureStage(page, testInfo, "sample-final-report");
