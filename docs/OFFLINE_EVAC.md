...
地図パックと道路グラフ・経路・名前・保存日・出典・メモをIndexedDBの1レコードへ、単一トランザクションで保存する。リザルト内のプレビュー時に地図取得を済ませ、保存ボタンで端末内に確定する。同じ出発地点・避難先・災害種別・練習／実データ区分は、体験日時が違っても同じマップとして更新する（座標は小数5桁）。既存の重複も最新の1件へ統合し、以前のIDは別名として保持して古いリンクを開けるようにする。削除と新しい保存は単一トランザクションで、更新なら5件の上限に達していても保存できる。失敗時は以前の記録を残す。
