...
APIキーが無効・未設定でも、トップの「サンプルで最後まで体験する（API不要）」または `/demo` から、写真確認 → 危険候補 → クイズ → 結果・シェア → 避難ルート → 最終レポートまで進めます。
サンプル写真とダウンロード用データ（`/demo/room.json`）はアプリに同梱しています。デモ中はOpenAIもGoogle Mapsも呼びません。結果画像にはサンプルの元写真を使い、その旨を表示します。
通常の写真解析も、接続失敗・無応答時は解析12秒／画像30秒を上限にサンプル結果・写真へのマーカー表示へ切り替わります。
