...
通常の避難経路のAPI体験は、接続確認済みのStreet Viewを8ノード進むごとに出題します。問題数の上限はなく、迂回後も8ノードごとに続きます。8ノード未満の区間では出題せず、到着が8ノード目なら回答後に完了します。歩行中の想定問題30種類は、状況と優先順位を考える3択で、表示順は毎回変わります。共通問題を使い切った長い経路では、一巡ごとに再利用します。固定のモック・サンプル体験は従来の練習用配置です。
